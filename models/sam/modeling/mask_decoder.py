...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.segmentation_head = nn.Conv2d(in_channels = transformer_dim // 8, out_channels = 5, kernel_size=3, padding=1)
    ...
    def forward(
        ...
        logits = F.interpolate(masks, scale_factor=4, mode='bilinear')
    ...
    def predict_masks(
        ...
        # masks = self.segmentation_head(upscaled_embedding)
        b, c, h, w = x.shape
        logits = self.logit_scale.exp() * x.permute(0, 2, 3, 1).view(b, -1, c).float() @ text_embeddings.transpose(1, 2).float()
        b, _, n = logits.shape
